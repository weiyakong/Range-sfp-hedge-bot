...
The approved 5m localization artifact is `macro_pivots_5m_all.csv` with SHA-256:
...
The candidate set therefore contains 145 distinct 5m intervals: 59 spot and 86 USDT-M futures.
...
It refines only the time/sequence location of the approved anchor price inside approved candidate 5m interval(s).

### Requirement: Same-market official trade evidence is used

For every candidate 5m interval, use the matching Binance market population:
...
For each macro anchor, scan every approved candidate 5m interval and collect every eligible source record whose exact `price_units` equals the anchor `price_units`.

Refinement statuses include at minimum:

- `exact_unique_trade_touch`: exactly one eligible touch across all candidate intervals and complete required source coverage;
...
A 5m candle being unique does not imply the trade touch is unique.
...
For multiple touches, `exact_pivot_time` and exact sequence id remain null. Preserve touch count, every touch row/key, first/last touch time, first/last candidate 5m interval, and an explicit ambiguity status.
...
### Requirement: Unique pivot creates deterministic LEFT and RIGHT 5m fragments

For an exact pivot inside canonical 5m interval `[B0,B1)` with ordered pivot record key `K` and pivot price `P`:

- LEFT temporal fragment = `[B0,pivot]` for price reconstruction and contains source records with ordering key `<= K`;
- RIGHT temporal fragment = `[pivot,B1)` for price reconstruction and contains source records with ordering key `> K`;
...
For exact LEFT/RIGHT 5m fragments preserve, where source semantics permit:
...
- underlying trade count only where it is exactly derivable from source-native ids;
...
Trade-level fragment path is a separate microstructure feature family.

For an exact macro leg from pivot `(t0,P0)` to `(t1,P1)`, the canonical close-path sequence at calculation resolution `R` is:

- `Q0=P0`;
- then chronological closes of complete canonical `R` candles with `t0 < candle.end_time <= t1`;
- finally append `P1` if the last sequence value is not already the exact end-anchor price/state.

The same Q sequence defines displacement, directional close-path components, alternation, and close-path efficiency at that resolution. No pre-pivot portion of the start-containing candle and no post-pivot close of the end-containing candle is introduced.
...
The 5m partial piece comes from the trade refinement. Any remaining portion between the adjacent 5m boundary and the enclosing `R` boundary is composed only from complete canonical 5m candles.

This permits exact boundary OHLCV/geometry/activity at higher resolutions without downloading trades for the whole 15m/1H/4H/1D interval.
...
- macro exact duration/speed and exact boundary-inclusive metrics that depend on one pivot remain null;
