...
### Requirement: Canonical analytical storage is Parquet or equivalent columnar storage

Full-resolution canonical market data, atomic pair records, large feature tables, and other large analytical datasets SHALL be retained in Parquet or another explicitly approved columnar analytical format suitable for deterministic filtering and joins.

This includes the retained canonical `1m` OHLCV/provenance layer and the complete target-resolution candle/feature tables.

Canonical analytical storage SHALL preserve schema/version, stable join keys, provenance, temporal coverage, and partition metadata needed for deterministic reconstruction.
...
A deterministic extraction utility SHALL be able to read the canonical Parquet store and export selected rows/columns/time ranges/feature families to CSV.
...
For partitioned canonical or review tables, manifests SHALL identify logical table name, part filenames/paths, row counts, temporal coverage, schema/version, source segment where applicable, and sufficient metadata to reconstruct the logical table deterministically.

### Requirement: Resume state does not redefine canonical output

Checkpoint, cache, and resume artifacts MAY use Parquet or another repository-approved internal format but SHALL remain distinguishable from validated canonical analytical outputs.
