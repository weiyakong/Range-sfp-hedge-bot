...
def _apply_smoke_window(config: BuildConfig, daily: pd.DataFrame, h4: pd.DataFrame, m15: pd.DataFrame, canonical: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    canonical_smoke = canonical[(canonical["start_time"] <= end) & (canonical["end_time"] >= start)].copy()
...
def _coverage(legs: pd.DataFrame, bars: pd.DataFrame, timeframe: str, market_source: str) -> pd.DataFrame:
    ...
    for leg in legs.to_dict(orient="records"):
        ...
        rows.append({"canonical_leg_id": leg["canonical_leg_id"], "timeframe": timeframe, "coverage_start": subset["open_datetime"].min() if actual_rows else pd.NaT, "coverage_end": subset["close_datetime"].max() if actual_rows else pd.NaT, "expected_rows": expected_rows, "actual_rows": actual_rows, "coverage_share": actual_rows / expected_rows if expected_rows else 0.0, "gap_count": gap_count, "is_partial_start": bool(actual_rows and subset["open_datetime"].min() > leg["start_time"]), "is_partial_end": bool(actual_rows and subset["close_datetime"].max() < leg["end_time"]), "market_source": market_source, "coverage_status": status if actual_rows else "timeframe_unavailable"})
...
def main() -> int:
    ...
    descriptors = [SourceDescriptor(config.daily_parquet, "1d", "merged", "explicit"), SourceDescriptor(config.merged_h4_parquet, "4h", "merged", "explicit"), SourceDescriptor(config.spot_h4_parquet, "4h", "spot", "explicit"), SourceDescriptor(config.futures_h4_parquet, "4h", "futures", "explicit"), SourceDescriptor(config.merged_15m_parquet, "15m", "merged", "explicit"), SourceDescriptor(config.spot_15m_parquet, "15m", "spot", "explicit"), SourceDescriptor(config.futures_15m_parquet, "15m", "futures", "explicit")]
    ...
    canonical, memberships = canonicalize_legs({"macro_legs_log20": upstream["macro_legs_log20"], "movement_segments_log20_fibtime": upstream["movement_segments_log20_fibtime"]})
    ...
    coverage = pd.concat([_coverage(canonical, market_bars_1d, "1d", "merged"), _coverage(canonical, market_bars_4h, "4h", "merged"), _coverage(canonical, market_bars_15m, "15m", "merged")], ignore_index=True)
    outputs = {"market_bars_1d.parquet": market_bars_1d, "market_bars_4h.parquet": market_bars_4h, "market_bars_15m.parquet": market_bars_15m, "market_features_rolling_4h_causal.parquet": rolling, "dynamic_range_candidates_4h_causal.parquet": candidates, "dynamic_range_excursions_4h.parquet": excursions, "structure_leg_bars_1d_retrospective.parquet": leg_bars_1d, "structure_leg_bars_4h_retrospective.parquet": leg_bars_4h, "structure_leg_bars_15m_retrospective.parquet": leg_bars_15m, "structure_leg_features_static.csv": canonical.merge(previous, left_on="canonical_leg_id", right_on="current_leg_id", how="left"), "structure_leg_features_rolling_4h_retrospective.parquet": leg_bars_4h, "structure_leg_events_retrospective.parquet": leg_events, "structure_canonical_legs.csv": canonical, "structure_source_memberships.csv": memberships, "decision_parent_reference_candidates.csv": candidate_table, "structure_leg_relationships.csv": relationships, "fibtime_event_duplicate_audit.csv": duplicate_audit, "fibtime_events_v4.csv": fibtime, "coverage_report.csv": coverage}
    ...
    checkpoint_entries = {"config_hash": config.config_hash(), "partials": {"causal_4h": save_partial(rolling, config.output_dir / "partials/causal_4h/2026-07.parquet"), "dynamic_candidates": save_partial(candidates, config.output_dir / "partials/dynamic_candidates/2026-07.parquet"), "dynamic_excursions": save_partial(excursions, config.output_dir / "partials/dynamic_excursions/2026-07.parquet"), "decision_support": save_partial(decision_tables["decision_window_comparison.csv"], config.output_dir / "partials/decision_support/2026-07.parquet")}}
