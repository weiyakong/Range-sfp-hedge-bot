...
class StructureResearchV4Tests(unittest.TestCase):
    ...
    def _sample_bars(self, freq: str = "4h", periods: int = 120) -> pd.DataFrame:
        ...
        for i in range(periods):
            ...
            rows.append({"interval": freq, "open_datetime": open_time, "close_datetime": close_time, "open": price - 0.5, "high": price + 1.0, "low": price - 1.0, "close": price, "volume": 10 + i})
    ...
    def test_main_smoke_integration(self) -> None:
        ...
        cmd = ["python3", str(ROOT / "build_structure_research_dataset_v4.py"), "--base-run-dir", str(upstream), "--daily-parquet", str(daily_path), "--daily-merge-summary", str(manifest), "--merged-h4-parquet", str(h4_path), "--spot-h4-parquet", str(h4_path), "--futures-h4-parquet", str(h4_path), "--merged-15m-parquet", str(m15_path), "--spot-15m-parquet", str(m15_path), "--futures-15m-parquet", str(m15_path), "--aggtrades-root", str(self.temp_dir), "--parquet-manifest", str(manifest), "--parquet-schema", str(schema), "--output-dir", str(output_dir), "--mode", "smoke"]
